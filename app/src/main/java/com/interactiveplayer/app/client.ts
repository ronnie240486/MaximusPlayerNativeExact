...
//
// Testando o endpoint direto (GET .../heartbeat.php?mac=...) no painel de
// produção: ele sempre responde 200 com o ÚLTIMO valor já salvo, mesmo
// mandando `content` diferente por query string — ou seja, o GET é só
// leitura. Não dá pra ter certeza de qual formato de POST o backend
// realmente espera (e isso roda em ~12 painéis parecidos, possivelmente
// com pequenas diferenças), então em vez de confiar cegamente no primeiro
// formato, tenta alguns em sequência e CONFERE de verdade com um GET de
// volta se o conteúdo foi mesmo gravado antes de desistir.
let lastWorkingHeartbeatStrategy = 0;

async function heartbeatStrategies(url: string, mac: string, content: string): Promise<Array<() => Promise<boolean>>> {
  return [
    async () => {
    ...
    async () => {
      const res = await fetch(proxied(url), {
        ...
        body: `mac=${encodeURIComponent(mac)}&content=${encodeURIComponent(content)}`,
      });
      return res.ok;
    },
    async () => {
      const res = await fetch(proxied(`${url}?mac=${encodeURIComponent(mac)}&content=${encodeURIComponent(content)}`), {
...
async function verifyHeartbeat(url: string, mac: string, expected: string): Promise<boolean> {
  try {
    const res = await fetch(proxied(`${url}?mac=${encodeURIComponent(mac)}`), { headers: commonHeaders });
    const json = await safeJson<any>(res);
    return !!json && typeof json.content === 'string' && json.content.trim().toLowerCase() === expected.trim().toLowerCase();
  } catch {
    return false;
  }
}

async function sendHeartbeatAt(base: string, mac: string, content: string): Promise<boolean> {
  const url = `${base}/heartbeat.php`;
  const strategies = await heartbeatStrategies(url, mac, content);
  const order = [...strategies.keys()].slice(lastWorkingHeartbeatStrategy).concat([...strategies.keys()].slice(0, lastWorkingHeartbeatStrategy));
  for (const index of order) {
    try {
      if (await strategies[index]() && (await verifyHeartbeat(url, mac, content))) {
        lastWorkingHeartbeatStrategy = index;
        return true;
      }
    } catch {
...
export async function sendHeartbeat(mac: string, content: string): Promise<void> {
  try {
    if (await sendHeartbeatAt(PANEL_BASE_V4, mac, content)) return;
    await sendHeartbeatAt(PANEL_BASE_V4_FALLBACK, mac, content);
  } catch {
